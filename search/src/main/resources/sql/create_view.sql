create view `search`.`user_info` AS
 select
   ...
   `pui`.`is_trainer`,
   `pui`.`is_looking_for_trainer`,
   `pui`.`is_blogger`,
    AVG(`r`.`rating`) as `rating`,
    COUNT(distinct `r`.`id`) as `review_count`,
    GROUP_CONCAT(distinct (select name from `profile`.`medical_expertise` where `id` = `ume`.`medical_expertise_id`)) as `user_medical_expertise`,
    GROUP_CONCAT(distinct (select name from `profile`.`yoga_certificates` where `id` = `uyc`.`yoga_certificate_id`)) as `user_yoga_certificate`,
    GROUP_CONCAT(distinct (select name from `profile`.`medical_expertise` where `id` = `uye`.`yoga_expertise_id`)) as `user_yoga_expertise`
FROM
`user`.`users` `u`
