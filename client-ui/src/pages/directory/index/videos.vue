...
export default {
  ...
  async preFetch({ store, currentRoute, previousRoute, redirect, ssrContext }) {
    ...
    let data = store.getters["directory_videos/byPage"](
  ...
  computed: {
    sub() {
      return this.$store.getters["directory_videos/byPage"](
  ...
  methods: {
    getTitle() {
      return `Watch yoga videos shared by people accross the world | JustYoga`;
    },
    getDesc() {
      return "Watch yoga videos shared by people accross the world on justyoga.fit. Yoga trainers share videos here.";
    },
    getUrl() {
      return `${config.uiUrl}/directory/videos/${this.$route.params.page}`;
    },
    getImage() {
    ...
    getPrevLink() {
      if (this.sub.hasPrevious)
        return `${config.uiUrl}/directory/videos/${+this.sub.page - 1}`;
      else null;
    },
    getNextLink() {
      if (this.sub.hasNext)
