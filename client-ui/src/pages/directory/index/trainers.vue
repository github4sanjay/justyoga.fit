...
export default {
  name: "DirectoryTrainers",
  components: {
    DirectoryTrainerSection
  },
  async preFetch({ store, currentRoute, previousRoute, redirect, ssrContext }) {
    Loading.show();
    let data = store.getters["directory/byPage"](currentRoute.params.page);
    if (!data) {
      try {
  ...
  computed: {
    sub() {
      return this.$store.getters["directory/byPage"](this.$route.params.page);
    }
  },
  methods: {
    getTitle() {
      return `Find Yoga Trainers accross the world | JustYoga`;
    },
    getDesc() {
      return "Find the best yoga trainers accross the world on justyoga.fit. Trainers register themselves here.";
    },
    getUrl() {
      return `${config.uiUrl}/directory/trainers/${this.$route.params.page}`;
    },
    getImage() {
    ...
    getPrevLink() {
      if (this.sub.hasPrevious)
        return `${config.uiUrl}/directory/trainers/${+this.sub.page - 1}`;
      else null;
    },
    getNextLink() {
      if (this.sub.hasNext)
