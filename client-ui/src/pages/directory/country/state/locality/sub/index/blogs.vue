...
export default {
  ...
  async preFetch({ store, currentRoute, previousRoute, redirect, ssrContext }) {
    Loading.show();
    let data = store.getters["directory_blogs/subLocalityLevel1ByPage"](
  ...
  computed: {
    sub() {
      return this.$store.getters["directory_blogs/subLocalityLevel1ByPage"](
        this.$route.params.sub,
  ...
  methods: {
    getTitle() {
      return `Read yoga blogs shared by people in ${this.subLocality.name}, ${this.locality.name}, ${this.state.name}, ${this.country.name} | JustYoga`;
    },
    getDesc() {
      return "Read yoga blogs shared by people accross the world on justyoga.fit. Yoga trainers share blogs and articles here.";
    },
    getUrl() {
      return `${config.uiUrl}/directory/${this.country.id}/${this.state.id}/${this.locality.id}/${this.subLocality.id}/blogs/${this.$route.params.page}`;
    },
    getImage() {
    ...
    getPrevLink() {
      if (this.sub.hasPrevious)
        return `${config.uiUrl}/directory/${this.country.id}/${this.state.id}/${
          this.locality.id
        }/${this.subLocality.id}/blogs/${+this.sub.page - 1}`;
      else null;
    },
    getNextLink() {
      if (this.sub.hasNext)
