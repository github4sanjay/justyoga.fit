...
export default {
  ...
  computed: {
    sub() {
      return this.$store.getters["directory_images/subLocalityLevel1ByPage"](
        this.$route.params.sub,
  ...
  methods: {
    getTitle() {
      return `Browse yoga photos shared by people in ${this.subLocality.name}, ${this.locality.name}, ${this.state.name}, ${this.country.name} | JustYoga`;
    },
    getDesc() {
    ...
    getUrl() {
      return `${config.uiUrl}/directory/${this.country.id}/${this.state.id}/${this.locality.id}/${this.subLocality.id}/images/${this.$route.params.page}`;
    },
    getImage() {
    ...
    getPrevLink() {
      if (this.sub.hasPrevious)
        return `${config.uiUrl}/directory/${this.country.id}/${this.state.id}/${
          this.locality.id
        }/${this.subLocality.id}/images/${+this.sub.page - 1}`;
      else null;
    },
    getNextLink() {
      if (this.sub.hasNext)
