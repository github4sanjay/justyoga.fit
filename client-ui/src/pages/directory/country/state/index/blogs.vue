...
export default {
  ...
  async preFetch({ store, currentRoute, previousRoute, redirect, ssrContext }) {
    Loading.show();
    let data = store.getters["directory_blogs/administrativeAreaByPage"](
  ...
  computed: {
    sub() {
      return this.$store.getters["directory_blogs/administrativeAreaByPage"](
        this.$route.params.state,
  ...
  methods: {
    getTitle() {
      return `Read yoga blogs shared by people in ${this.state.name}, ${this.country.name} | JustYoga`;
    },
    getDesc() {
      return "Read yoga blogs shared by people accross the world on justyoga.fit. Yoga trainers share blogs and articles here.";
    },
    getUrl() {
      return `${config.uiUrl}/directory/${this.country.id}/${this.state.id}/blogs/${this.$route.params.page}`;
    },
    getImage() {
