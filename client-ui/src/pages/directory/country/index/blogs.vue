...
export default {
  ...
  computed: {
    sub() {
      return this.$store.getters["directory_blogs/countryByPage"](
  ...
  methods: {
    getTitle() {
      return `Read yoga blogs shared by people in ${this.country.name} | JustYoga`;
    },
    getDesc() {
      return "Read yoga blogs shared by people accross the world on justyoga.fit. Yoga trainers share blogs and articles here.";
    },
    getUrl() {
      return `${config.uiUrl}/directory/${this.country.id}/blogs/${this.$route.params.page}`;
    },
    getImage() {
    ...
    getPrevLink() {
      if (this.sub.hasPrevious)
        return `${config.uiUrl}/directory/${this.country.id}/blogs/${+this.sub
          .page - 1}`;
      else null;
    },
    getNextLink() {
