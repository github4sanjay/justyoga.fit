...
export default {
  ...
  async preFetch({ store, currentRoute, previousRoute, redirect, ssrContext }) {
    Loading.show();
    let data = store.getters["directory_images/countryByPage"](
  ...
  computed: {
    sub() {
      return this.$store.getters["directory_images/countryByPage"](
  ...
  methods: {
    getTitle() {
      return `Browse yoga photos shared by people in ${this.country.name} | JustYoga`;
    },
    getDesc() {
    ...
    getUrl() {
      return `${config.uiUrl}/directory/${this.country.id}/images/${this.$route.params.page}`;
    },
    getImage() {
    ...
    getPrevLink() {
      if (this.sub.hasPrevious)
        return `${config.uiUrl}/directory/${this.country.id}/images/${+this.sub
          .page - 1}`;
      else null;
    },
    getNextLink() {
