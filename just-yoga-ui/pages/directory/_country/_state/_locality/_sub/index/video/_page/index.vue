...
export default {
  ...
  middleware: "directory_sub1_video",
  data: function () {
    return {
      sub: this.$store.getters["directory_videos/subLocalityLevel1ByPage"](
  ...
  computed: {
    prevTo() {
      return `/directory/${this.country}/${this.state}/${this.locality}/${this.subLocalityLevel1}/video/${this.previousPage}`;
    },
    nextTo() {
      return `/directory/${this.country.id}/${this.state.id}/${this.locality.id}/${this.subLocalityLevel1}/video/${this.nextPage}`;
    },
  },
};
</script>
